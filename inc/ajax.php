...
/**
 * Gợi ý tìm kiếm AJAX: tiêu đề phim, danh mục (channels), diễn viên (pornstars).
 */
...
function twf_search_suggestions() {
    ...
    if (!is_wp_error($terms_cat) && !empty($terms_cat)) {
        foreach ($terms_cat as $t) {
            ...
            $channels[] = array(
                'label' => $t->name,
                // Trang archive taxonomy danh mục
                'url'   => $link,
            );
        }
    }
    ...
    if (!is_wp_error($terms_act) && !empty($terms_act)) {
        foreach ($terms_act as $t) {
            ...
            $pornstars[] = array(
                'label' => $t->name,
                // Trang diễn viên (taxonomy ophim_actors)
                'url'   => $link,
            );
        }
    }
