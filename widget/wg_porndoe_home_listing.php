<?php
class WG_PornDoe_Home_Listing extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'wg_porndoe_home_listing',
            __( 'Home Video Listing', 'ophim' ),
            array( 'description' => __( 'Danh sách phim trang chủ với tab link và phân trang mới', 'ophim' ) )
        );
    }
    ...
    public function widget( $args, $instance ) {
        ...
        $primary_url = $instance['primary_url'];
        $secondary_label = $instance['secondary_label'];
        $secondary_url = $instance['secondary_url'];
        ...
        $primary_url = $primary_url ? $primary_url : home_url('/');
        $secondary_url = $secondary_url ? $secondary_url : home_url('/most-viewed');
        $single_row = $postnum <= 6;
        ?>
        <section class="-section">
            <div class="-holder -flex">
                <div class="-grow">
                    <h2 class="-heading-one">
                        <a href="<?php echo esc_url($primary_url); ?>" title="<?php echo esc_attr($title); ?>">
                            <?php echo esc_html($title); ?>
                        </a>
                    </h2>
                </div>
                <div class="-items-center">
                    <a class="home-more-desktop" href="<?php echo esc_url($secondary_url); ?>">
                        +More Videos
                    </a>
                </div>
            </div>
        ...
        <?php if (!empty($primary_label) || !empty($secondary_label)) : ?>
            <section class="-section home-inline-nav">
                <?php if (!empty($primary_label)) : ?>
                    <a class="<?php echo $orderby === 'new' ? 'active' : ''; ?>" href="<?php echo esc_url($primary_url); ?>">
                        <?php echo esc_html($primary_label); ?>
                    </a>
                <?php endif; ?>
                <?php if (!empty($secondary_label)) : ?>
                    <a class="<?php echo $orderby === 'view' ? 'active' : ''; ?>" href="<?php echo esc_url($secondary_url); ?>">
                        <?php echo esc_html($secondary_label); ?>
                    </a>
                <?php endif; ?>
                <a class="<?php echo $orderby === 'rate' ? 'active' : ''; ?>" href="<?php echo esc_url(add_query_arg('sort', 'likes', $secondary_url)); ?>">
                    Most Liked
                </a>
                <a class="<?php echo $orderby === 'rand' ? 'active' : ''; ?>" href="<?php echo esc_url(add_query_arg('sort', 'length', $secondary_url)); ?>">
                    Longest
                </a>
            </section>
        <?php endif; ?>
        ...
        <section class="-section">
            <div class="-holder">
                <?php if ($query->have_posts()) : ?>
                    <?php if ($single_row) : ?>
                        <div class="videos-category-carousel">
                    <?php endif; ?>
                    <div class="videos-list videos-list-large-thumbs" data-single-row="<?php echo $single_row ? 'true' : 'false'; ?>">
                        <?php
                        $rank = 0;
                ...
                <?php endif; ?>
                <div class="-section">
                    <a class="home-more-mobile" href="<?php echo esc_url($secondary_url); ?>">
                        View All Videos
                    </a>
                </div>
            <?php else : ?>
                <div class="-holder-section">
                    <?php esc_html_e('Chưa có video mới.', 'ophim'); ?>
                </div>
            <?php endif; ?>
    ...
    function form($instance)
    {
        ...
        <p>
            <label for="<?php echo $this->get_field_id('primary_label'); ?>"><?php _e('Label tab 1', 'ophim') ?></label>
            <br />
            <input class="widefat" type="text" id="<?php echo $this->get_field_id('primary_label'); ?>" name="<?php echo $this->get_field_name('primary_label'); ?>" value="<?php echo esc_attr($instance['primary_label']); ?>" />
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('primary_url'); ?>"><?php _e('URL tab 1', 'ophim') ?></label>
            <br />
            <input class="widefat" type="text" id="<?php echo $this->get_field_id('primary_url'); ?>" name="<?php echo $this->get_field_name('primary_url'); ?>" value="<?php echo esc_attr($instance['primary_url']); ?>" />
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('secondary_label'); ?>"><?php _e('Label tab 2', 'ophim') ?></label>
            <br />
            <input class="widefat" type="text" id="<?php echo $this->get_field_id('secondary_label'); ?>" name="<?php echo $this->get_field_name('secondary_label'); ?>" value="<?php echo esc_attr($instance['secondary_label']); ?>" />
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('secondary_url'); ?>"><?php _e('URL tab 2', 'ophim') ?></label>
            <br />
            <input class="widefat" type="text" id="<?php echo $this->get_field_id('secondary_url'); ?>" name="<?php echo $this->get_field_name('secondary_url'); ?>" value="<?php echo esc_attr($instance['secondary_url']); ?>" />
        </p>
    ...
    public function update( $new_instance, $old_instance ) {
        ...
        $instance['primary_label'] = ( ! empty( $new_instance['primary_label'] ) ) ? strip_tags( $new_instance['primary_label'] ) : '';
        $instance['primary_url'] = ( ! empty( $new_instance['primary_url'] ) ) ? esc_url_raw( $new_instance['primary_url'] ) : '';
        $instance['secondary_label'] = ( ! empty( $new_instance['secondary_label'] ) ) ? strip_tags( $new_instance['secondary_label'] ) : '';
        $instance['secondary_url'] = ( ! empty( $new_instance['secondary_url'] ) ) ? esc_url_raw( $new_instance['secondary_url'] ) : '';
...
function register_porndoe_home_listing_widget() {
    register_widget( 'WG_PornDoe_Home_Listing' );
}
add_action( 'widgets_init', 'register_porndoe_home_listing_widget' );
